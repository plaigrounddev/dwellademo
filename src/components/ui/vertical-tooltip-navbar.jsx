...
export const TooltipVerticalNavbar = ({
  ...
  const calculatePosition = (index) => {
    ...
    const totalHeight = measureRefs.current.reduce((acc, el) => acc + (el?.offsetHeight || 0), 0);
  ...
  return (
    <div className="flex h-screen items-center px-8">
      <div className="relative text-white" onMouseLeave={handleMouseLeave}>
                    ...
                    <div
                      ...
                      {item.hasBadge && (
                        <div className="flex items-center gap-0.5 text-white/40">
                          <span
                            className="flex items-center justify-center rounded-sm border border-white/20 p-1">
        ...
        <div
          className="z-10 flex flex-col items-center justify-center rounded-full bg-black/95 p-2 backdrop-blur dark:bg-neutral-800">
          {items.map((item, index) => (
            <button
              key={index}
              ...
              <div
                className="flex size-10 items-center justify-center p-1.5 dark:text-neutral-200">
                {item.icon}
              </div>
      ...
      <div
        className="pointer-events-none absolute top-0 left-0 flex flex-col overflow-hidden whitespace-nowrap opacity-0">
        {items.map((item, index) => (
          <div
            key={`measure-${index}`}
            ref={(el) => {
