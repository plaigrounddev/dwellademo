...
const metadata = await readBuilderWebsiteUpdateProposalConvexImportMetadata(options.dataDir);
const paths = artifactPaths(options.dataDir);
let rows = 0;

if (options.dryRun) {
  for await (const batch of readNdjsonMappedBatches(paths.websiteUpdateProposals, toConvexWebsiteUpdateProposal, options.batchSize)) {
    rows += batch.length;
  }
  assertExpectedImportRows("builder website update proposal Convex dry run", rows, metadata.expectedRows);
...
await runWithRetry("website update proposal run manifest", () => client.mutation(api.builderMemory.upsertWebsiteUpdateProposalRun, metadata.websiteUpdateProposalRun));

let batchNumber = 0;
for await (const batch of readNdjsonMappedBatches(paths.websiteUpdateProposals, toConvexWebsiteUpdateProposal, options.batchSize)) {
  batchNumber += 1;
  await runWithRetry(`website update proposal batch ${batchNumber}`, () =>
    client.mutation(api.builderMemory.upsertWebsiteUpdateProposalBatch, {
      importId: metadata.importId,
      proposals: batch,
    })
  );
  rows += batch.length;
  if (!options.quiet && (batchNumber === 1 || batchNumber % 100 === 0)) console.error(`websiteUpdateProposals: ${rows} rows imported`);
}

assertExpectedImportRows("builder website update proposal Convex import", rows, metadata.expectedRows);
await runWithRetry("website update proposal import finalization", () =>
  client.mutation(api.builderMemory.finalizeWebsiteUpdateProposalRun, { importId: metadata.importId, loadedRows: rows })
);
...
function usage(error) {
  ...
  console.error(`Usage:
  node scripts/import-builder-website-update-proposals-to-convex.mjs --dry-run [--data-dir data/builders]
  CONVEX_URL=https://... node scripts/import-builder-website-update-proposals-to-convex.mjs [--data-dir data/builders] [--batch-size 100]`);
