...
if (options.dryRun) {
  for await (const batch of readNdjsonMappedBatches(paths.detailedListRows, toConvexDetailedListRow, options.batchSize)) {
    rows += batch.length;
  }
  assertExpectedImportRows("builder detailed list Convex dry run", rows, metadata.expectedRows);
  printJson({
    status: "dry_run_ok",
    importId: metadata.importId,
    generatedAt: metadata.detailedListRun.generatedAt,
...
await runWithRetry("detailed list run manifest", () => client.mutation(api.builderMemory.upsertDetailedListRun, metadata.detailedListRun));

let batchNumber = 0;
for await (const batch of readNdjsonMappedBatches(paths.detailedListRows, toConvexDetailedListRow, options.batchSize)) {
  batchNumber += 1;
  await runWithRetry(`detailed list row batch ${batchNumber}`, () =>
    client.mutation(api.builderMemory.upsertDetailedListRowBatch, {
      importId: metadata.importId,
      rows: batch,
    })
  );
  ...
  if (!options.quiet && (batchNumber === 1 || batchNumber % 100 === 0)) {
    console.error(`detailedListRows: ${rows} rows imported`);
  }
}

assertExpectedImportRows("builder detailed list Convex import", rows, metadata.expectedRows);
await runWithRetry("detailed list import finalization", () =>
  client.mutation(api.builderMemory.finalizeDetailedListRun, {
    importId: metadata.importId,
    loadedRows: rows,
  })
);
...
function usage(error) {
  ...
  console.error(`Usage:
  node scripts/import-builder-detailed-list-to-convex.mjs --dry-run [--data-dir data/builders] [--batch-size 100]
  CONVEX_URL=https://... node scripts/import-builder-detailed-list-to-convex.mjs [--data-dir data/builders] [--batch-size 100] [--max-retries 5]`);
