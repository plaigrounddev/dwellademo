...
if (options.dryRun) {
  for await (const batch of readNdjsonMappedBatches(paths.websiteCandidates, toConvexWebsiteCandidate, options.batchSize)) {
    rows += batch.length;
  }
  assertExpectedImportRows("builder website candidate Convex dry run", rows, metadata.expectedRows);
...
await runWithRetry("website candidate run manifest", () => client.mutation(api.builderMemory.upsertWebsiteCandidateRun, metadata.websiteCandidateRun));

let batchNumber = 0;
for await (const batch of readNdjsonMappedBatches(paths.websiteCandidates, toConvexWebsiteCandidate, options.batchSize)) {
  batchNumber += 1;
  await runWithRetry(`website candidate batch ${batchNumber}`, () =>
    client.mutation(api.builderMemory.upsertWebsiteCandidateBatch, {
      importId: metadata.importId,
      candidates: batch,
    })
  );
  rows += batch.length;
  if (!options.quiet && (batchNumber === 1 || batchNumber % 100 === 0)) console.error(`websiteCandidates: ${rows} rows imported`);
}

assertExpectedImportRows("builder website candidate Convex import", rows, metadata.expectedRows);
await runWithRetry("website candidate import finalization", () =>
  client.mutation(api.builderMemory.finalizeWebsiteCandidateRun, { importId: metadata.importId, loadedRows: rows })
);
...
function usage(error) {
  ...
  console.error(`Usage:
  node scripts/import-builder-website-candidates-to-convex.mjs --dry-run [--data-dir data/builders]
  CONVEX_URL=https://... node scripts/import-builder-website-candidates-to-convex.mjs [--data-dir data/builders] [--batch-size 100]`);
