...
const metadata = await readBuilderWebsiteDiscoveryConvexImportMetadata(options.dataDir);
const paths = artifactPaths(options.dataDir);
let jobRows = 0;

if (options.dryRun) {
  for await (const batch of readNdjsonMappedBatches(paths.websiteDiscoveryJobs, toConvexWebsiteDiscoveryJob, options.batchSize)) {
    jobRows += batch.length;
  }
  assertExpectedImportRows("builder website discovery Convex dry run", jobRows, metadata.expectedJobRows);
  printJson({
    status: "dry_run_ok",
    importId: metadata.importId,
    generatedAt: metadata.websiteDiscoveryRun.generatedAt,
...
await runWithRetry("website discovery run manifest", () =>
  client.mutation(api.builderMemory.upsertWebsiteDiscoveryRun, metadata.websiteDiscoveryRun)
);

let batchNumber = 0;
for await (const batch of readNdjsonMappedBatches(paths.websiteDiscoveryJobs, toConvexWebsiteDiscoveryJob, options.batchSize)) {
  batchNumber += 1;
  await runWithRetry(`website discovery job batch ${batchNumber}`, () =>
    client.mutation(api.builderMemory.upsertWebsiteDiscoveryJobBatch, {
      importId: metadata.importId,
      jobs: batch,
    })
  );
  ...
  if (!options.quiet && (batchNumber === 1 || batchNumber % 100 === 0)) {
    console.error(`websiteDiscoveryJobs: ${jobRows} rows imported`);
  }
}

assertExpectedImportRows("builder website discovery Convex import", jobRows, metadata.expectedJobRows);
await runWithRetry("website discovery import finalization", () =>
  client.mutation(api.builderMemory.finalizeWebsiteDiscoveryRun, {
    importId: metadata.importId,
...
function usage(error) {
  ...
  console.error(`Usage:
  node scripts/import-builder-website-discovery-jobs-to-convex.mjs --dry-run [--data-dir data/builders] [--batch-size 100]
  CONVEX_URL=https://... node scripts/import-builder-website-discovery-jobs-to-convex.mjs [--data-dir data/builders] [--batch-size 100] [--max-retries 5]`);
