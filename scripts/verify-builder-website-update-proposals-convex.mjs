...
if (options.mockResultFile) {
  ...
  try {
    verification = await client.query(api.builderMemory.verifyWebsiteUpdateProposalRun, { importId: options.importId ?? metadata.importId });
  } catch (error) {
    printJson({
      status: "unavailable",
      ...
      failures: [
        `Convex website update proposal verification query failed: ${error instanceof Error ? error.message : String(error)}`,
        "Deploy the Convex builderMemory functions before verifying a live website update proposal import.",
      ],
    });
    process.exit(1);
  }
...
const localCheck = verifyConvexWebsiteUpdateProposalRun(metadata.expectedRows, verification);
const result = {
  status: localCheck.ok && verification?.ok ? "ok" : "mismatch",
  importId: verification?.importId ?? options.importId ?? metadata.importId,
  generatedAt: verification?.generatedAt ?? metadata.websiteUpdateProposalRun.generatedAt,
  source: verification?.source ?? metadata.websiteUpdateProposalRun.source,
  expectedRows: metadata.expectedRows,
  loadedRows: verification?.loadedRows ?? null,
  totals: verification?.totals ?? null,
  proposalsSha256: verification?.proposalsSha256 ?? null,
...
function usage(error) {
  ...
  console.error(`Usage:
  CONVEX_URL=https://... node scripts/verify-builder-website-update-proposals-convex.mjs [--data-dir data/builders]
  node scripts/verify-builder-website-update-proposals-convex.mjs --mock-result-file verification.json [--data-dir data/builders]`);
