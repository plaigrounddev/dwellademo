...
if (options.mockResultFile) {
  ...
  try {
    verification = await client.query(api.builderMemory.verifyEnrichmentRun, { importId: options.importId ?? metadata.importId });
  } catch (error) {
    printJson({
      status: "unavailable",
      ...
      failures: [
        `Convex enrichment verification query failed: ${error instanceof Error ? error.message : String(error)}`,
        "Deploy the Convex builderMemory functions before verifying a live enrichment import.",
      ],
    });
    process.exit(1);
  }
...
const localCheck = verifyConvexEnrichmentRun(metadata.expectedJobRows, verification);
const result = {
  status: localCheck.ok && verification?.ok ? "ok" : "mismatch",
  importId: verification?.importId ?? options.importId ?? metadata.importId,
  generatedAt: verification?.generatedAt ?? metadata.enrichmentRun.generatedAt,
  sourceManifestGeneratedAt: verification?.sourceManifestGeneratedAt ?? metadata.enrichmentRun.sourceManifestGeneratedAt,
  expectedJobRows: metadata.expectedJobRows,
  loadedJobRows: verification?.loadedJobRows ?? null,
  gapCounts: verification?.gapCounts ?? null,
  suggestedJobCounts: verification?.suggestedJobCounts ?? null,
...
function usage(error) {
  ...
  console.error(`Usage:
  CONVEX_URL=https://... node scripts/verify-builder-enrichment-convex.mjs [--data-dir data/builders]
  node scripts/verify-builder-enrichment-convex.mjs --mock-result-file verification.json [--data-dir data/builders]`);
