...
if (options.dryRun) {
  for await (const batch of readNdjsonMappedBatches(paths.websiteSearchRequests, toConvexWebsiteSearchRequest, options.batchSize)) rows += batch.length;
  assertExpectedImportRows("builder website search request Convex dry run", rows, metadata.expectedRows);
  printJson({
    status: "dry_run_ok",
    importId: metadata.importId,
    generatedAt: metadata.websiteSearchRequestRun.generatedAt,
...
await runWithRetry("website search request run manifest", () => client.mutation(api.builderMemory.upsertWebsiteSearchRequestRun, metadata.websiteSearchRequestRun));

let batchNumber = 0;
for await (const batch of readNdjsonMappedBatches(paths.websiteSearchRequests, toConvexWebsiteSearchRequest, options.batchSize)) {
  batchNumber += 1;
  await runWithRetry(`website search request batch ${batchNumber}`, () =>
    client.mutation(api.builderMemory.upsertWebsiteSearchRequestBatch, {
      importId: metadata.importId,
      requests: batch,
    })
  );
  rows += batch.length;
  if (!options.quiet && (batchNumber === 1 || batchNumber % 500 === 0)) {
    console.error(`websiteSearchRequests: ${rows} rows imported`);
  }
}

assertExpectedImportRows("builder website search request Convex import", rows, metadata.expectedRows);
await runWithRetry("website search request import finalization", () =>
  client.mutation(api.builderMemory.finalizeWebsiteSearchRequestRun, {
    importId: metadata.importId,
    loadedRows: rows,
  })
);
...
function usage(error) {
  ...
  console.error(`Usage:
  node scripts/import-builder-website-search-requests-to-convex.mjs --dry-run [--data-dir data/builders] [--batch-size 100]
  CONVEX_URL=https://... node scripts/import-builder-website-search-requests-to-convex.mjs [--data-dir data/builders] [--batch-size 100] [--max-retries 5]`);
