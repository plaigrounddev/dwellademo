...
if (options.mockResultFile) {
  ...
  try {
    verification = await client.query(api.builderMemory.verifyProductionReadinessRun, { importId: options.importId ?? metadata.importId });
  } catch (error) {
    printJson({
      status: "unavailable",
      importId: options.importId ?? metadata.importId,
      failures: [
        `Convex production readiness verification query failed: ${error instanceof Error ? error.message : String(error)}`,
        "Deploy the Convex builderMemory functions before verifying a live production readiness import.",
      ],
    });
    process.exit(1);
  }
...
const localCheck = verifyConvexProductionReadinessRun(metadata.productionReadinessRun, verification);
const result = {
  status: localCheck.ok && verification?.ok ? "ok" : "mismatch",
  importId: verification?.importId ?? options.importId ?? metadata.importId,
  generatedAt: verification?.generatedAt ?? metadata.productionReadinessRun.generatedAt,
  readinessStatus: verification?.readinessStatus ?? null,
  expectedStatus: metadata.productionReadinessRun.status,
  summary: verification?.summary ?? null,
  blockerCount: verification?.blockerCount ?? null,
  blockers: (verification?.blockers ?? []).map((blocker) => blocker.blockerId),
...
function usage(error) {
  ...
  console.error(`Usage:
  CONVEX_URL=https://... node scripts/verify-builder-production-readiness-convex.mjs [--data-dir data/builders]
  node scripts/verify-builder-production-readiness-convex.mjs --mock-result-file verification.json [--data-dir data/builders]`);
